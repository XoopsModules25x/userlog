...
class UserlogSetting extends XoopsObject
{
	...
	public function getOptions($option = null, $V = "value")
	{
		...
		"outside" => array("type" => "int",
								"title" => _AM_USERLOG_OUTSIDE,
								"value" => 1
								),
		"unset_pass" => array("type" => "int",
		...
		foreach ($ret as $option=>$val) {
			...
			if (!$emptyAll || in_array($option, array("log_id","log_time","active","inside","outside","unset_pass","store_file","store_db","views"))) continue;
			// check values
...
class UserlogSettingHandler extends XoopsPersistableObjectHandler
{
   ...
   /**
    ...
    {
        $this->userlog = Userlog::getInstance();
         parent::__construct($db, "mod_" . USERLOG_DIRNAME . "_set", 'UserlogSetting', "set_id", "logby");
	}
