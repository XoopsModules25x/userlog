...
class UserlogLog extends XoopsObject
{
	...
	public function arrayToDisplay($logs, $skips = array())
	{
		foreach($logs as $log_id=>$log) {
			...
			foreach($this->_sourceJSON as $option) {
				...
				if($option == "env") break; // only $_sourceJSON = array("get","post","request","files","env" 
			}
	...
	public function setItem()
	{
		...
		include_once dirname(__FILE__) . "/plugin/Abstract.php";
		...
		if (!empty($not_config)) {
			foreach ($not_config['category'] as $category) {
				...
				if (!empty($category['item_name']) && 
						...
						$item_id = UserlogRequest::getInt($category['item_name'], 0)
					) {
...
class UserlogLogHandler extends XoopsPersistableObjectHandler
{
   ...
   /**
    ...
    {
        $this->userlog = Userlog::getInstance();
        parent::__construct($db, "mod_" . USERLOG_DIRNAME . "_log", 'UserlogLog', "log_id", "log_time");
	}
     ...
     * @param array $ret[my_field] = Field	Type	Null	Key		Default		Extra
     */
	...
	{
		if(empty($field) || empty($structure)) return false;
		if($this->showFields($field)) return false; // field is exist
	...
	public function dropField($field = null)
	{
		...
		if(!$this->showFields($field)) return false; // field is not exist
    ...
    {
		if(empty($table)) $table = $this->table; // the table for this object
		// check if database prefix is not added yet and then add it!!!
		if(strpos($table, $this->db->prefix() . "_") !== 0) $table = $this->db->prefix("{$table}");
    ...
    {
		if ($this->showTable() || !$this->showTable($oldTable)) return false; // table is current || oldTable is not exist
		// check if database prefix is not added yet and then add it!!!
		if(strpos($oldTable, $this->db->prefix() . "_") !== 0) $oldTable = $this->db->prefix("{$oldTable}");
